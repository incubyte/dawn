...
// Create a global function to handle play/pause toggling
// This allows both the button click and keyboard shortcut to use the same code
let togglePlayPauseAudio: (() => Promise<void>) | null = null;
...
function setupTransportHandlers(audioEngine?: AudioEngine): void {
    ...
    togglePlayPauseAudio = async () => {
        try {
            if (!audioEngine) return;

            console.log(`Toggle play/pause called, current state: ${audioEngine.isPlaying ? 'playing' : 'paused'}`);
    ...
    if (playToggleButton && audioEngine) {
        playToggleButton.addEventListener('click', () => {
            ...
            if (togglePlayPauseAudio) togglePlayPauseAudio();
        });
    }
...
function setupKeyboardShortcuts(audioEngine?: AudioEngine): void {
    ...
    const playToggleButton = document.getElementById('play-toggle-button');
    if (!audioEngine || !playToggleButton) return;
    ...
    document.addEventListener('keyup', (event) => {
        ...
        if (event.code === 'Space' || event.key === ' ') {
            ...
            if (togglePlayPauseAudio) {
                ...
                console.error('Toggle play/pause function not initialized');
            }
        }
    });
